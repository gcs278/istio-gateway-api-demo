...
oc get secret router-certs-default -n openshift-ingress -o "jsonpath={.data['tls\.key']}" | base64 -d > /tmp/openshift-default.key
oc get secret router-certs-default -n openshift-ingress -o "jsonpath={.data['tls\.crt']}" | base64 -d > /tmp/openshift-default.crt
